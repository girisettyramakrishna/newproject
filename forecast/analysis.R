analysisUI = function(id,label ="analysis"){
  ...
  tagList(
    ...
    tabsetPanel(tabPanel(title = "EDA",
                ...
                tabPanel(title = "Time Series Analysis",
                         tabsetPanel(
                           tabPanel(title = "Percentage change over time",highchartOutput(ns("per_change"))),
                           tabPanel(title = "Sales Decomposition",
                                    fluidRow(
                                      column(width = 4,div("Seasonality",style="font-size:20px;font-style:bold;"),highchartOutput(ns("decompose_seasonal"))),
                                      column(width = 4,div("Trend",style="font-size:20px;font-style:bold;"),highchartOutput(ns("decompose_trend"))),
                                      column(width = 4,div("Randomness",style="font-size:20px;font-style:bold;"),highchartOutput(ns("decompose_random")))
                                    ),
                                    fluidRow(column(4,tableOutput(ns("summary_seasonal"))),column(4,tableOutput(ns("summary_trend"))),column(4,tableOutput(ns("summary_random"))))
                                    
                                    ),
                           tabPanel(title = "Seasonal Analysis",
...
######################
##country wise########
data_arima_country= data.frame(data_arima[,c(9,1)])
...
analysis=function(input,output,session,timeline,exo,year1,year2,year3,fcType,nWeeks,type,store_wise,reg_wise,seasonality){
  ...
  decomp = function(data,x,y){
    ...
    decomposition = (decompose(data_procedureA))
    ...
    if(x == 1){
      ...
      library(xts)
      pc_MONTH = xts(decomp_overall$seasonality,order.by=decomp_overall$Date)
      hc <- highchart(type = "stock") %>% 
        hc_add_series(pc_MONTH, id = "Seasonality") %>% 
        hc_title(text = paste0("Seasonality"))
      # hc <- highchart(type = "stock") %>%
      ...
      #trend and date
      library(xts)
      pc_MONTH = xts(decomp_overall$trend,order.by=decomp_overall$Date)
      hc <- highchart(type = "stock") %>% 
        hc_add_series(pc_MONTH, id = "trend") %>% 
        hc_title(text = paste0("trend"))
      # hc <- highchart(type = "stock") %>%
      ...
      #   hc_legend(enabled = T)    
        ...
        #randomness and date
        pc_MONTH = xts(decomp_overall$randomness,order.by=decomp_overall$Date)
        hc <- highchart(type = "stock") %>% 
          hc_add_series(pc_MONTH, id = "randomness") %>% 
          hc_title(text = paste0("randomness"))
         # hc <- highchart(type = "stock") %>%
  ...
  decomp_summary = function(data_arima_SS2,x,y){
    ...
    if(x == 1){
      df= (data.frame(unclass(summary(procedureA_decomp$seasonal))))
    } else if(x==2){
      df= (data.frame(unclass(summary(procedureA_decomp$trend))))
      
    } else if(x==3){
      df= (data.frame(unclass(summary(procedureA_decomp$random))))
  ...
  output$decompose_seasonal = renderHighchart({
    if(type() == "Country"){
      if(fcType() == "ARIMA"){
        if(seasonality() == "Quarterly"){
  ...
  output$decompose_trend = renderHighchart({
    if(type() == "Country"){
      if(fcType() == "ARIMA"){
        if(seasonality() == "Quarterly"){
  ...
  output$decompose_random = renderHighchart({
    if(type() == "Country"){
      if(fcType() == "ARIMA"){
        if(seasonality() == "Quarterly"){
