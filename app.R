...
server <- function(input, output, session) {
  ...
  observeEvent(input$.login, {
    if (isTRUE(credentials[[input$.username]]==input$.password)){
      ...
      #delay(000.1, hide("message", anim = TRUE, animType = "fade"))
    }
  })
    ...
    if (isTRUE(USER$Logged)) {
      dashboardBody(
        ...
        tabsetPanel(
          ...
          tabPanel(title="Univariate and Bivariate Analysis",
          ...
          tabPanel(title="Multivariate",
                   forecastUI("forecast")),
          tabPanel(title="Outcomes and Reports",
      ...
      fluidRow(column(width=2, offset = 5,br(),br(),br(),br(),br(),br(),br(),br(),
                      wellPanel(id = "login",
